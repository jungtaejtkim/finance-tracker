class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present? #userstrock 테이블을 찾아봐라
      @user_stock = UserStock.new(stock_id: params[:stock_id], user: current_user)

    else
      stock = Stock.find_by_ticker(params[:stock_ticker]) #없으면 stock 테이블에있나 봐라 
      if stock
        @user_stock = UserStock.new(user: current_user, stock: stock)
      else 
        stock = Stock.new_from_lookup(params[:stock_ticker]) #그것도 없으면 Stock gem 을 활용해서 밖에서 불러와서 맺어라 
        if stock.save
          @user_stock=UserStock.new(user: current_user, stock: stock) #밖에서 불러온걸로 유저스탁을 만들어라
        else
          @user_stock=nil
